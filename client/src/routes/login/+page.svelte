...
<div
  class="sm:p-0 px-3 py-8 w-full h-full grid place-items-center bg-gradient-to-b from-indigo-9 to-indigo-950"
>
  <div
    class="bg-zinc-9 max-w-lg max-h-2xl w-full h-full rounded-5 overflow-x-hidden
  ...
  >
    ...
    <div class="flex flex-col gap-6">
      ...
      <div
        class="{loginFailed
          ? 'flex'
          : 'display-none'} text-sm text-red-5 gap-2 items-center"
      >
        <span class="i-heroicons:exclamation-triangle-20-solid text-lg" />
        <span> Nome de usuário ou senha inválido(s). </span>
      </div>
      {#if isSignup}
        <div class="flex flex-col gap-4">
          <div class="font-semibold">Escolha seu avatar</div>
          <div
            class="grid justify-evenly grid-flow-col text-white
            ...
            {#each Avatars as avatar, i}
              <img
                ...
                class="{avatarId === i ? 'border-indigo-6' : 'border-zinc-5'}
                  sm:w-20 sm:h-20 w-15 h-15 rounded-full object-cover border-2 border-solid transition-250"
              />
            {/each}
          </div>
        </div>
      ...
      <div class="flex flex-col gap-3">
        ...
        <div class="w-full grid grid-cols-2 gap-2">
          ...
          >
            <div class="sm:block display-none i-heroicons-user-solid" />
          ...
          <div
            class="px-6 py-3 rounded-lg border-solid border-2 border-indigo-8 items-center
                    text-center cursor-pointer hover:brightness-80 transition-250 flex gap-2 justify-center"
          >
            <div class="sm:block display-none i-heroicons-lock-closed-20-solid" />
